...
describe('CreateStreamModal - Empty State Edge Cases', () => {
  ...
  it('should reset to step 1 with all fields cleared when navigating back from step 3', () => {
    ...
    expect(recipientContainer).not.toHaveClass('input-container--error');
    expect(recipientContainer).not.toHaveClass('input-container--success');
    expect(depositContainer).not.toHaveClass('input-container--error');
    expect(depositContainer).not.toHaveClass('input-container--success');
  });
  
  it('should reject submission when form is completely empty', () => {
    const { container } = openSingleStreamWizard();
    ...
    expect(screen.getByText(/deposit must be positive/i)).toBeInTheDocument();
  });
  
  it('should show error on field blur, not on change, when field becomes empty', () => {
    const { container } = openSingleStreamWizard();
    ...
    expect(screen.queryByText(/rate must be positive/i)).not.toBeInTheDocument();
