...
/**
 * Builds the storage key for a given account address.
 * Each wallet address gets its own isolated session data.
 */
function getStorageKey(accountAddress: string): string {
  // Normalize to prevent case-sensitivity issues (Stellar addresses are case-sensitive
  // but we want consistent storage keys)
  const normalized = accountAddress.trim();
  return `${STREAMS_SESSION_STORAGE_KEY_PREFIX}_${normalized}`;
}
...
function parseSnapshot(raw: string): StreamsSessionSnapshot | null {
  ...
  return { savedAt: parsed.savedAt, accountAddress: parsed.accountAddress, filters, draft };
}
...
export function readStreamsSession(
  ...
  if (!storage || !accountAddress || accountAddress.trim() === "") return null;
...
export function writeStreamsSession(
  ...
  if (!storage || !accountAddress || accountAddress.trim() === "") return;

  try {
    const storageKey = getStorageKey(accountAddress);
    const full: StreamsSessionSnapshot = { ...snapshot, savedAt: now, accountAddress };
